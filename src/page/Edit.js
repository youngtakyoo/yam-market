import React from "react";

import { Grid, Text, Input, Button, Image } from '../elements'
import { Forbidden, Upload } from "../components";

import { useSelector, useDispatch } from 'react-redux';
import { history } from "../redux/configureStore";
import { actionCreators as postActions } from "../redux/modules/Post";
import { actionCreators as imgActions } from "../redux/modules/Image";

const Edit = (props) => {
    // 로그인 여부 체크
    const is_login = useSelector((state)=> state.user.is_login);
    const dispatch = useDispatch();
    // 편집여부 체크
    const is_edit = props.match.params.post_id ? true : false;
    // 프리뷰 체크
    let length = useSelector((state)=> state.image.preview);
    length = length.length;
    // 프리뷰 이미지 배열
    let previews = useSelector(state => state.image.preview);

    // 작성 기능
    const [_title,setTitle] = React.useState('');
    const [_desc,setDesc] = React.useState('');

    const posting = () => {
        if(_title.length === 0 || _desc.length === 0 || previews.length === 0){
            window.alert('내용이 빈곳이 있습니다.')
            return
        }
        dispatch(postActions.addPost({title:_title,desc:_desc}))
        history.push('/');
    }

    if(!is_login){
        return(
            <Forbidden />
        )
    }

    return (
        <Grid >
            <Grid padding='16px' margin='10px auto'  is_flex>
                <Text bold size='36px'>게시글 수정</Text>
            </Grid>
            <Grid width='50%' padding='16px' margin='10px auto' is_flex is_column>
                <Grid width='100%' padding='0 16px 16px' >
                    <Input value={_title} _onChange={(e)=>{setTitle(e.target.value)}} non_label placeholder='제목을 입력해주세요.' />
                </Grid>

                <Grid width='80%' is_flex padding='10px'>
                    <Grid width='60%' border padding='5px' is_flex is_column>    
                        <Upload />
                        <Image src={previews[0] ? previews[0] : "http://via.placeholder.com/400x300"} />
                        { length > 0 &&
                        <Button _onClick={()=>{dispatch(imgActions.delPreview(previews[0]))}} bg='#fff' >삭제</Button>}
                    </Grid>
                    { length > 0 && 
                    <Grid width='60%' border padding='5px' is_flex is_column>    
                        <Upload />
                        <Image src={previews[1] ? previews[1] : "http://via.placeholder.com/400x300" } />
                        { length > 1 &&
                        <Button _onClick={()=>{dispatch(imgActions.delPreview(previews[1]))}} bg='#fff' >삭제</Button>}
                    </Grid>}
                    { length > 1 && 
                    <Grid width='60%' border padding='5px' is_flex is_column>    
                        <Upload />
                        <Image src={previews[2] ? previews[2] : "http://via.placeholder.com/400x300" }/>
                        { length > 2 &&
                        <Button _onClick={()=>{dispatch(imgActions.delPreview(previews[2]))}} bg='#fff' >삭제</Button>}
                    </Grid>}
                </Grid>
            </Grid>
            <Grid padding='10px' is_flex is_column>
                <Input value={_desc} _onChange={(e)=>{setDesc(e.target.value)}} is_area placeholder='내용을 입력해주세요.' />
                <Grid width='auto' padding='10px'>
                    <Button _onClick={posting} width='80px' bg='#fff' >{is_edit ? '수정' : '작성'}</Button>
                </Grid>
            </Grid>
        </Grid>
    )
}

export default Edit;